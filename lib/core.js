...
// if the code of the character is greater than or equal to 255(2 Bytes), the width of the character is twice as wide as 1 byte character. 
function getByteLength(text){
  let byteLength = 0;
  for(let i = 0; i < text.length; i++){
    byteLength += text[i].charCodeAt(0) > 255 ? 2 : 1;
  }
  return byteLength;
}

function findSubstringIndex(cellWidth, text){
  let byteLength = 0;
  for(let i = 0; i < text.length; i++){
    byteLength += text[i].charCodeAt(0) > 255 ? 2 : 1;
    if(byteLength > cellWidth){
      return i;
    }
  }
  return text.legnth;
...
class ThermalPrinter {
  ...
  // ----------------------------------------------------- TABLE -----------------------------------------------------
  table(data) {
    var cellWidth = parseInt(this.config.width / data.length, 10);
    for (var i = 0; i < data.length; i++) {
      this.append(data[i].toString());
      var spaces = cellWidth - getByteLength(data[i].toString());
  ...
  tableCustom(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (obj.width) {
        cellWidth = parseInt(this.config.width * obj.width, 10);
      } else if (obj.cols) {
        cellWidth = obj.cols
      }
      ...
      const substringIndex = findSubstringIndex(cellWidth, obj.text);
      if (cellWidth < getByteLength(obj.text)) {
        tooLong = true;
        obj.originalText = obj.text;
        obj.text = obj.text.substring(0, substringIndex);
      }

      if (obj.align == "CENTER") {
        var spaces = (cellWidth - getByteLength(obj.text.toString())) / 2;
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        ...
        var spaces = cellWidth - getByteLength(obj.text.toString());
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        if (obj.text != '') this.append(obj.text);
      ...
      if (tooLong) {
        ...
        obj.text = obj.originalText.substring(substringIndex);
        secondLine.push(obj);
      } else {
        obj.text = "";
        secondLine.push(obj);
