...
class ThermalPrinter {
  ...
  table(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var spaces = cellWidth - getTextByteLength(data[i].toString());
  ...
  tableCustom(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (cellWidth < getTextByteLength(obj.text)) {
        tooLong = true;
      ...
      if (obj.align == "CENTER") {
        var spaces = (cellWidth - getTextByteLength(obj.text.toString())) / 2;
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        ...
        var spaces = cellWidth - getTextByteLength(obj.text.toString());
        for (var j = 0; j < spaces; j++) {
          this.append(Buffer.from(" "));
        }
        if (obj.text != '') this.append(obj.text);
